...
def ensure_pyaterochka_schema(engine):
    """Создает схему pyaterochka если она не существует"""
    try:
        with engine.connect() as conn:
            # Проверяем существование схемы pyaterochka
            check_schema_sql = """
            SELECT COUNT(*) 
            FROM sys.schemas 
            WHERE name = 'pyaterochka'
            """
            result = conn.execute(text(check_schema_sql)).scalar()
            
            if result == 0:
                # Создаем схему pyaterochka
                create_schema_sql = "CREATE SCHEMA pyaterochka"
                conn.execute(text(create_schema_sql))
                logging.info("Схема pyaterochka создана")
            else:
                logging.info("Схема pyaterochka уже существует")
                
            return True
            
    except Exception as e:
        logging.error(f"Ошибка создания схемы pyaterochka: {e}")
        return False

def create_table_if_not_exists(engine, table_name, sample_chunk):
    """Создает таблицу в схеме pyaterochka если она не существует"""
    
    # Сначала убеждаемся, что схема pyaterochka существует
    if not ensure_pyaterochka_schema(engine):
        return False
    ...
    IF NOT EXISTS (SELECT * FROM sys.tables t 
                   ...
                   WHERE t.name = '{table_name}' AND s.name = 'pyaterochka')
    BEGIN
        CREATE TABLE [pyaterochka].[{table_name}] (
            {', '.join(columns_sql)}
        )
    END
    """
    ...
    try:
        ...
        logging.info(f"Таблица pyaterochka.{table_name} создана или уже существует")
        return True
        
    except Exception as e:
        logging.error(f"Ошибка создания таблицы pyaterochka.{table_name}: {e}")
        return False
...
def safe_to_sql_fixed(df, table_name, engine, schema='pyaterochka', if_exists='append'):
            ...
            if not chunk.empty:
                ...
                if first_chunk:
                    if create_table_if_not_exists(engine, table_name, enriched_chunk):
                        logging.info(f"Таблица pyaterochka.{table_name} готова")
                    first_chunk = False
                ...
                if safe_to_sql_fixed(enriched_chunk, table_name, engine, if_exists='replace' if chunk_num == 0 else 'append'):
                    ...
                    logging.info(f"УСПЕШНО загружен чанк {chunk_num} в pyaterochka.{table_name}")
                else:
...
def process_file_complete(file_path):
    ...
    try:
        ...
        if total_rows > 0:
            ...
            logging.info(f"Таблица: pyaterochka.{table_name}")
            
            # Проверяем что данные действительно в базе
            try:
                with engine.connect() as conn:
                    result = conn.execute(text(f"SELECT COUNT(*) FROM pyaterochka.{table_name}"))
                    count_in_db = result.scalar()
                    logging.info(f"ПРОВЕРКА: В таблице pyaterochka.{table_name} сейчас {count_in_db} строк")
                    
                    # Проверяем несколько записей
                    sample_result = conn.execute(text(f"SELECT TOP 3 product_name, brand_predicted, city_predicted FROM pyaterochka.{table_name}"))
...
with DAG(
    dag_id="pyaterochka_sales_pipeline_complete",
    default_args=default_args,
    start_date=datetime(2025, 1, 1),
    schedule_interval="@daily",
    catchup=False,
    max_active_tasks=1,
    tags=["pyaterochka", "complete", "all_models"],
) as dag:
