...
def ensure_diksi_schema(engine):
    """Создает схему diksi если она не существует"""
    try:
        with engine.connect() as conn:
            # Проверяем существование схемы diksi
            check_schema_sql = """
            SELECT COUNT(*) 
            FROM sys.schemas 
            WHERE name = 'diksi'
            ...
            if result == 0:
                # Создаем схему diksi
                create_schema_sql = "CREATE SCHEMA diksi"
                conn.execute(text(create_schema_sql))
                logging.info("Схема diksi создана")
            else:
                logging.info("Схема diksi уже существует")
                
            return True
            
    except Exception as e:
        logging.error(f"Ошибка создания схемы diksi: {e}")
        return False

def create_table_if_not_exists(engine, table_name, sample_chunk):
    """Создает таблицу в схеме diksi если она не существует"""
    
    # Сначала убеждаемся, что схема diksi существует
    if not ensure_diksi_schema(engine):
        return False
    ...
    IF NOT EXISTS (SELECT * FROM sys.tables t 
                   ...
                   WHERE t.name = '{table_name}' AND s.name = 'diksi')
    BEGIN
        CREATE TABLE [diksi].[{table_name}] (
            {', '.join(columns_sql)}
        )
    END
    """
    ...
    try:
        ...
        logging.info(f"Таблица diksi.{table_name} создана или уже существует")
        return True
        
    except Exception as e:
        logging.error(f"Ошибка создания таблицы diksi.{table_name}: {e}")
        return False
...
def safe_to_sql_fixed(df, table_name, engine, schema='diksi', if_exists='append'):
            ...
            if not chunk.empty:
                ...
                if first_chunk:
                    if create_table_if_not_exists(engine, table_name, enriched_chunk):
                        logging.info(f"Таблица diksi.{table_name} готова")
                    first_chunk = False
                ...
                if safe_to_sql_fixed(enriched_chunk, table_name, engine, if_exists='replace' if chunk_num == 0 else 'append'):
                    ...
                    logging.info(f"УСПЕШНО загружен чанк {chunk_num} в diksi.{table_name}")
                else:
...
@task
def scan_files():
    """Сканирует CSV файлы с префиксом diksi_ в директории"""
    csv_files = []
    try:
        for file in os.listdir(DATA_DIR):
            if file.startswith('diksi_') and file.endswith('.csv'):
...
def process_file_complete(file_path):
    ...
    try:
        ...
        if total_rows > 0:
            ...
            logging.info(f"Таблица: diksi.{table_name}")
            
            # Проверяем что данные действительно в базе
            try:
                with engine.connect() as conn:
                    result = conn.execute(text(f"SELECT COUNT(*) FROM diksi.{table_name}"))
                    count_in_db = result.scalar()
                    logging.info(f"ПРОВЕРКА: В таблице diksi.{table_name} сейчас {count_in_db} строк")
                    
                    # Проверяем несколько записей
                    sample_result = conn.execute(text(f"SELECT TOP 3 product_name, brand_predicted, city_predicted FROM diksi.{table_name}"))
...
with DAG(
    dag_id="diksi_sales_pipeline_complete",
    default_args=default_args,
    start_date=datetime(2025, 1, 1),
    schedule_interval="@daily",
    catchup=False,
    max_active_tasks=1,
    tags=["diksi", "complete", "all_models"],
) as dag:
