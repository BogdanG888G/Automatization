...
def ensure_okey_schema(engine):
    """Создает схему okey если она не существует"""
    try:
        with engine.connect() as conn:
            # Проверяем существование схемы okey
            check_schema_sql = """
            SELECT COUNT(*) 
            FROM sys.schemas 
            WHERE name = 'okey'
            ...
            if result == 0:
                # Создаем схему okey
                create_schema_sql = "CREATE SCHEMA okey"
                conn.execute(text(create_schema_sql))
                logging.info("Схема okey создана")
            else:
                logging.info("Схема okey уже существует")
                
            return True
            
    except Exception as e:
        logging.error(f"Ошибка создания схемы okey: {e}")
        return False

def create_table_if_not_exists(engine, table_name, sample_chunk):
    """Создает таблицу в схеме okey если она не существует"""
    
    # Сначала убеждаемся, что схема okey существует
            ...
            if not chunk.empty:
                ...
                if first_chunk:
                    if create_table_if_not_exists(engine, table_name, enriched_chunk):
                        logging.info(f"Таблица okey.{table_name} готова")
                    first_chunk = False
                ...
                if safe_to_sql_fixed(enriched_chunk, table_name, engine, if_exists='replace' if chunk_num == 0 else 'append'):
                    ...
                    logging.info(f"УСПЕШНО загружен чанк {chunk_num} в okey.{table_name}")
                else:
...
def process_file_complete(file_path):
    ...
    try:
        ...
        if total_rows > 0:
            ...
            logging.info(f"Таблица: okey.{table_name}")
            
            # Проверяем что данные действительно в базе
            try:
                with engine.connect() as conn:
                    result = conn.execute(text(f"SELECT COUNT(*) FROM okey.{table_name}"))
