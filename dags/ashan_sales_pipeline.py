...
def ensure_ashan_schema(engine):
    """Создает схему ashan если она не существует"""
    try:
        with engine.connect() as conn:
            # Проверяем существование схемы ashan
            check_schema_sql = """
            SELECT COUNT(*) 
            FROM sys.schemas 
            WHERE name = 'ashan'
            ...
            if result == 0:
                # Создаем схему ashan
                create_schema_sql = "CREATE SCHEMA ashan"
                conn.execute(text(create_schema_sql))
                logging.info("Схема ashan создана")
            else:
                logging.info("Схема ashan уже существует")
                
            return True
            
    except Exception as e:
        logging.error(f"Ошибка создания схемы ashan: {e}")
        return False

def create_table_if_not_exists(engine, table_name, sample_chunk):
    """Создает таблицу в схеме ashan если она не существует"""
    
    # Сначала убеждаемся, что схема ashan существует
    if not ensure_ashan_schema(engine):
        return False
    ...
    IF NOT EXISTS (SELECT * FROM sys.tables t 
                   ...
                   WHERE t.name = '{table_name}' AND s.name = 'ashan')
    BEGIN
        CREATE TABLE [ashan].[{table_name}] (
            {', '.join(columns_sql)}
        )
    END
    """
    ...
    try:
        ...
        logging.info(f"Таблица ashan.{table_name} создана или уже существует")
        return True
        
    except Exception as e:
        logging.error(f"Ошибка создания таблицы ashan.{table_name}: {e}")
        return False
...
def safe_to_sql_fixed(df, table_name, engine, schema='ashan', if_exists='append'):
            ...
            if not chunk.empty:
                ...
                if first_chunk:
                    if create_table_if_not_exists(engine, table_name, enriched_chunk):
                        logging.info(f"Таблица ashan.{table_name} готова")
                    first_chunk = False
                ...
                if safe_to_sql_fixed(enriched_chunk, table_name, engine, if_exists='replace' if chunk_num == 0 else 'append'):
                    ...
                    logging.info(f"УСПЕШНО загружен чанк {chunk_num} в ashan.{table_name}")
                else:
...
def process_file_complete(file_path):
    ...
    try:
        ...
        if total_rows > 0:
            ...
            logging.info(f"Таблица: ashan.{table_name}")
            
            # Проверяем что данные действительно в базе
            try:
                with engine.connect() as conn:
                    result = conn.execute(text(f"SELECT COUNT(*) FROM ashan.{table_name}"))
