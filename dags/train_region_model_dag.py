...
def train_region_model():
    ...
    df.dropna(subset=['address', 'region'], inplace=True)
    ...
    print(f"✅ Модель и векторизатор для region сохранены")
...
with DAG(
    dag_id='train_region_model_dag',
    default_args=default_args,
    schedule_interval=None,
    catchup=False,
    tags=['model_training', 'region'],
    description='Обучение модели для предсказания региона по адресу',
) as dag:

    train_model = PythonOperator(
        task_id='train_region_model',
        python_callable=train_region_model,
    )
