...
def ensure_perekrestok_schema(engine):
    """Создает схему perekrestok если она не существует"""
    try:
        with engine.connect() as conn:
            # Проверяем существование схемы perekrestok
            check_schema_sql = """
            SELECT COUNT(*) 
            FROM sys.schemas 
            WHERE name = 'perekrestok'
            """
            result = conn.execute(text(check_schema_sql)).scalar()
            
            if result == 0:
                # Создаем схему perekrestok
                create_schema_sql = "CREATE SCHEMA perekrestok"
                conn.execute(text(create_schema_sql))
                logging.info("Схема perekrestok создана")
            else:
                logging.info("Схема perekrestok уже существует")
                
            return True
            
    except Exception as e:
        logging.error(f"Ошибка создания схемы perekrestok: {e}")
        return False

def create_table_if_not_exists(engine, table_name, sample_chunk):
    """Создает таблицу в схеме perekrestok если она не существует"""
    
    # Сначала убеждаемся, что схема perekrestok существует
    if not ensure_perekrestok_schema(engine):
        return False
    ...
    IF NOT EXISTS (SELECT * FROM sys.tables t 
                   ...
                   WHERE t.name = '{table_name}' AND s.name = 'perekrestok')
    BEGIN
        CREATE TABLE [perekrestok].[{table_name}] (
            {', '.join(columns_sql)}
        )
    END
    """
    ...
    try:
        ...
        logging.info(f"Таблица perekrestok.{table_name} создана или уже существует")
        return True
        
    except Exception as e:
        logging.error(f"Ошибка создания таблицы perekrestok.{table_name}: {e}")
        return False
...
def safe_to_sql_fixed(df, table_name, engine, schema='perekrestok', if_exists='append'):
            ...
            if not chunk.empty:
                ...
                if first_chunk:
                    if create_table_if_not_exists(engine, table_name, enriched_chunk):
                        logging.info(f"Таблица perekrestok.{table_name} готова")
                    first_chunk = False
                ...
                if safe_to_sql_fixed(enriched_chunk, table_name, engine, if_exists='replace' if chunk_num == 0 else 'append'):
                    ...
                    logging.info(f"УСПЕШНО загружен чанк {chunk_num} в perekrestok.{table_name}")
                else:
...
def process_file_complete(file_path):
    ...
    try:
        ...
        if total_rows > 0:
            ...
            logging.info(f"Таблица: perekrestok.{table_name}")
            
            # Проверяем что данные действительно в базе
            try:
                with engine.connect() as conn:
                    result = conn.execute(text(f"SELECT COUNT(*) FROM perekrestok.{table_name}"))
                    count_in_db = result.scalar()
                    logging.info(f"ПРОВЕРКА: В таблице perekrestok.{table_name} сейчас {count_in_db} строк")
                    
                    # Проверяем несколько записей
                    sample_result = conn.execute(text(f"SELECT TOP 3 product_name, brand_predicted, city_predicted FROM perekrestok.{table_name}"))
