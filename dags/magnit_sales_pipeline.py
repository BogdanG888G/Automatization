...
@task
def scan_files():
    """Сканирует CSV файлы с префиксом magnit_ в директории"""
    csv_files = []
    try:
        for file in os.listdir(DATA_DIR):
            if file.startswith('magnit_') and file.endswith('.csv'):
...
with DAG(
    dag_id="magnit_sales_pipeline_complete",
    default_args=default_args,
    start_date=datetime(2025, 1, 1),
    schedule_interval="@daily",
    catchup=False,
    max_active_tasks=1,
    tags=["magnit", "complete", "all_models"],
) as dag:
